...
<?php
    ...
    function AddRmv($coluna, $acao, $campo, $tabela, $conexao) {
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST["add$acao"])) {
            if (isset($_POST[$campo]) && !empty($_POST[$campo])) {
                $novoItem = $_POST[$campo];
                $query_addItem = "INSERT INTO $tabela ($coluna) VALUES ('$novoItem')";
                mysqli_query($conexao, $query_addItem);
            }
            else{}
        }
        else{}

        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST["rmv$acao"])) {
            $removerItem = $_POST["remover$acao"];
            if (!empty($removerItem)) {
                $query_removerItem = "DELETE FROM $tabela WHERE $coluna = '$removerItem'";
                mysqli_query($conexao, $query_removerItem);
            }
            else{}
        }
        else{}
    ...
    AddRmv('veic', 'Veic', 'novoVeic', 'veics', $conexao);
    AddRmv('insp', 'Insp', 'novoInsp', 'insp', $conexao);
?>
<!DOCTYPE html>
<html lang="pt-br">
<head>
    ...
    <link rel="stylesheet" href="estilos/style-tabela_adm.css">
    ...
    <main>
        ...
        <section class="tabela">
            ...
            <div class="paginacao">
                ...
                for ($i = $pagina_inicial; $i <= $pagina_final; $i++) {
                    ...
                    if ($i == $pagina_atual) {
                        echo ' style="font-weight: bolder; border: 2.5px solid rgb(59, 59, 59); padding: 2px;"';
                    }
        ...
        <section class="addRmv">
            ...
            <div class="boxs">
                ...
                <div class="addRmv__box">
                    <form action="<?=$_SERVER['PHP_SELF']?>" method="post">
                        <input type="text" name="novoInsp" placeholder="Novo Inspetor">
                        <input type="submit" name="addInsp" value="Adicionar">
                    </form>
                    <form action="<?=$_SERVER['PHP_SELF']?>" method="post" >
                        <select name="removerInsp">
